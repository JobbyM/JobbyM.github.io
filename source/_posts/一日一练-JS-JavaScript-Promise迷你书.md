...
**接收的参数为其他类型的时候（包括JavaScript对象或null等）**
...
由于参数数组中的每个元素都是由 Promise.resolve 包装（wrap）的，所以Paomise.all 可以处理不同类型的 promise 对象。
...
**ES6 Promises**
    如果想明确表示使用 [ECMAScript 6th Edition](http://people.mozilla.org/~jorendorff/es6-draft.html#sec-promise-objects) 的话，可以使用ES6作为前缀（prefix）
